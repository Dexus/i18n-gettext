...
exports.start = function(options) {
  ...
  if (! options.translation_directory) options.translation_directory = 'locale/';
  ...
  function messages_file_path(locale) {
    ...
    var file_format = options.translation_type == 'po' ? 'po' : 'mo';
    var filename = 'messages.' + file_format;
    return path.resolve(path.join(__dirname, '..', '..', '..'),
                        options.translation_directory,
                        path.join(locale, 'LC_MESSAGES', filename));
  }

  function parse_messages_file(locale) {
    var filepath = messages_file_path(locale);
	var content = fs.readFile(filepath, function read(err, content) {
		if (err) {
			throw err;
		}
		dagt.addTextdomain(locale, content);
  ...
  return function(req, resp, next) {
    ...
    locals.setLocale = function(assignedLocale) {
      if (translations[assignedLocale]) {
		...
		dagt.textdomain(assignedLocale);
    ...
    locals.textdomain =  textdomain;
    req.textdomain = textdomain;
	locals.gettext =  gettext;
    req.gettext = gettext;
	locals.dgettext =  dgettext;
    req.dgettext = dgettext;
	locals.ngettext =  ngettext;
    req.ngettext = ngettext;
	locals.dngettext =  dngettext;
    req.dngettext = dngettext;
	locals.pgettext =  pgettext;
    req.pgettext = pgettext;
	locals.dpgettext =  dpgettext;
    req.dpgettext = dpgettext;
	locals.npgettext =  npgettext;
    req.npgettext = npgettext;
	locals.dnpgettext =  dnpgettext;
    req.dnpgettext = dnpgettext;
...
exports.textdomain = textdomain = function(domain){ return dagt.textdomain(domain);  };
exports.gettext = gettext = function(msgid){ return dagt.gettext(msgid); };
exports.dgettext = dgettext = function(domain, msgid){ return dagt.dgettext(domain, msgid) };
exports.ngettext = ngettext = function(msgid, msgidPlural, count){ return dagt.ngettext(msgid, msgidPlural, count) };
exports.dngettext = dngettext = function(domain, msgid, msgidPlural, count){ return dagt.dngettext(domain, msgid, msgidPlural, count) };
exports.pgettext = pgettext = function(context, msgid){ return dagt.pgettext(context, msgid) };
exports.dpgettext = dpgettext = function(domain, context, msgid){ return dagt.dpgettext(domain, context, msgid) };
exports.npgettext = npgettext = function(context, msgid, msgidPlural, count){ return dagt.npgettext(context, msgid, msgidPlural, count) };
exports.dnpgettext = dnpgettext = function(domain, context, msgid, msgidPlural, count){ return dagt.dnpgettext(domain, context, msgid, msgidPlural, count) };
